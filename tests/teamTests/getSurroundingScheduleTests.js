    ...
    it('returns 200 and valid surrounding schedule', function(done) {
        sendRequest(11, '10/15/2016', 20)
            .end(function(err, res) {
                ...
                assert.equal(17, res.body.nhl[0].awayTeamID);
                assert.equal('Anaheim Ducks', res.body.nhl[0].awayTeam);
                assert.equal(1, res.body.nhl[0].sportID);
                assert.equal('22:00:00', res.body.nhl[0].time);
                assert.equal('2016-10-16T04:00:00.000Z', res.body.nhl[0].date);
